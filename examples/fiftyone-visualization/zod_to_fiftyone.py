...
def create_dataset():
    ...
    # create pcd files directory if it doesn't exist
    if not os.path.exists(pcd_files_dir):
        os.makedirs(pcd_files_dir)
    ...
    if test_run:
        zod_frames_list = zod_frames_list[:50]
    ...
    if mapbox_token is not None:
        ...
        print("Mapbox token not found, map plugin not enbaled.")
